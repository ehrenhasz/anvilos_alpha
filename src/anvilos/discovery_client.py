...
class DiscoveryClient:
    ...
    def __init__(self, key_path=None):
        ...
        if not self.key_path:
            # Fallback to internal token if available
            internal_token = os.path.join(os.path.dirname(__file__), '../../config/token')
            if os.path.exists(internal_token):
                # Note: This token is usually an API Key, not SA creds. 
                # Discovery Engine often requires SA creds for indexing.
                # But for search (public), API key might work.
