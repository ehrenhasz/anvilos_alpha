...
class Collar:
    ...
    IGNORE_DIRS = [
        ".git", "node_modules", "dist", "build", "__pycache__", "venv", "oss_sovereignty"
    ]
    ...
    IGNORE_FILES = [
        "collar.py", "hydrogen.ts", "package-lock.json", "warden.py", "mainframe.js", "repl.js", "outline_engine.py" # Self-reference allowed for policing
    ]

    def check_active_card(self):
        db_path = "/var/lib/anvilos/db/cortex.db"
        if not os.path.exists(db_path):
            print(f"[COLLAR] Cortex DB not found at {db_path}")
            return False
        ...
        try:
            ...
            # Check sys_jobs (Big Iron) or card_stack (Mainframe)
            # We look for ANY pending or processing work to authorize operations
            
            # 1. Check Mainframe Stack
            cursor.execute("SELECT id, op, pld FROM card_stack WHERE stat IN (0, 1) LIMIT 1")
            row = cursor.fetchone()
            if row:
                print("=================================================")
                print(f"ACTIVE MAINFRAME CARD: {row[0]}")
                print("-------------------------------------------------")
                print(f"OP: {row[1]}")
                print("=================================================")
                conn.close()
            ...
            # 2. Check System Jobs
            cursor.execute("SELECT correlation_id, payload FROM sys_jobs WHERE status IN ('PENDING', 'PROCESSING') LIMIT 1")
            row = cursor.fetchone()
            if row:
                print("=================================================")
                print(f"ACTIVE JOB: {row[0]}")
                print("-------------------------------------------------")
            ...
            print("[COLLAR] No active cards or jobs found in Cortex.")
            conn.close()
            # Allow manual override if no cards are present, but warn
            # Actually, the original logic FAILED if no cards. 
            # "If len(active_cards) == 0: print... return False"
            # And stage_files calls sys.exit(1) if False.
            # So we must return False to maintain strictness, OR we allow it for manual commits?
            # RFC-043 says "ALWAYS run git_machine.sh". 
            # If I am running it manually, I might not have a card.
            # But the error message "ABORT: Collar violations detected" implies check_active_card returned False.
            return False

        except Exception as e:
            print(f"[COLLAR] Error during DB check: {e}")
            sys.exit(1)
