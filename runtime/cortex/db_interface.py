...
class CortexDB:
    ...
    def _init_db(self):
        with sqlite3.connect(self.db_path) as conn:
            # Instruction Stack
            # Added: agent_id, priority, parent_id for Lvl 2 Agency
            conn.execute("""
            ...
            # Migration: Check for missing columns in existing DB
    ...
    def pop_card(self) -> Optional[Dict[str, Any]]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Use RETURNING to get the card data and mark it as processing in one go.
            # This requires SQLite 3.35+
            query = """
                UPDATE card_stack 
                SET stat = 1 
                WHERE id = (
            ...
            try:
                ...
                # Fallback if RETURNING is not supported or other issue
                if "RETURNING" in str(e):
                    # Manual locking/transaction fallback (less robust but works on older sqlite)
    ...
    def peek_stack(self) -> Optional[Dict[str, Any]]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.row_factory = sqlite3.Row
            # Order by Priority (DESC), then Time (ASC)
            row = conn.execute("SELECT * FROM card_stack WHERE stat = 0 ORDER BY priority DESC, timestamp ASC, seq ASC LIMIT 1").fetchone()
            if row:
                return dict(row)
        return None
