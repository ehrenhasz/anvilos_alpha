...
# Use both locations just in case, but primary is /var/lib/anvilos/db/cortex.db
SYSTEM_DB = "/var/lib/anvilos/db/cortex.db"
LOCAL_DB = os.path.join(PROJECT_ROOT, "runtime", "cortex.db")

def init_mainframe(db_path):
    if not os.path.exists(os.path.dirname(db_path)):
        print(f"[WARN] Directory for {db_path} does not exist. Skipping.")
        return
