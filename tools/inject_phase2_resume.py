...
CARDS = [
    {
        "op": "SYS_CMD",
        "pld": {
            "cmd": "rm -rf build_artifacts/rootfs_stage build_artifacts/iso build_artifacts/bin build_artifacts/initramfs.cpio.gz",
            "timeout": 60
        },
    ...
    },
    {
        "op": "SYS_CMD",
        "pld": {
            "cmd": "cd oss_sovereignty/zfs-2.2.2 && export PKG_CONFIG_PATH=$(pwd)/../staging/lib/pkgconfig && ./configure --with-config=user --enable-static=yes --enable-shared=no --prefix=/ --libdir=/lib --sbindir=/sbin --with-libuuid=$(pwd)/../staging && make -j$(nproc)",
            "timeout": 1200
        },
