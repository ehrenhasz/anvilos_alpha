...
def inject():
    ...
    cards = [
        ...
        ("BUILD_ZFS_USERLAND", 
         ...
         f"LDFLAGS='-L{STAGING}/lib' "
         f"CPPFLAGS='-I{STAGING}/include -I{STAGING}/include/tirpc' "
         f"./configure --prefix=/usr --sysconfdir=/etc --localstatedir=/var --enable-linux-builtin=yes --with-linux={PROJECT_ROOT}/oss_sovereignty/linux-6.6.14 --enable-static --disable-shared --with-config=user && "
         f"make -j$(nproc)"),
        ("PREP_KERNEL",
        ...
        ("BUILD_ZFS_MODULES",
         f"cd {PROJECT_ROOT}/oss_sovereignty/zfs-2.2.2 && "
         f"./configure --with-linux={PROJECT_ROOT}/oss_sovereignty/linux-6.6.14 --with-linux-obj={PROJECT_ROOT}/oss_sovereignty/linux-6.6.14 --with-config=kernel && "
    ...
    conn.commit()
    conn.close()
    print(f"[*] Injecting {len(cards)} Phase 2 (FIXED V8) Cards...")
