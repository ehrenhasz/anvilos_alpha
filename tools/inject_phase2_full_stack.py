...
def inject():
    ...
    cards = [
        ...
        # --- 2. USERLAND FORGING (DEPENDENCIES) ---
        ...
        ("4. ROOTFS", "POPULATE_ROOTFS_POOL",
         # Import pool to temp mount, rsync files, export
        ...
        ("5. INIT", "WRITE_INIT_SCRIPT",
         ...
         # Assume rootfs.img is available (e.g., from ISO)
         # For now, simple initramfs shell
        ...
        # Need isolinux.bin etc? Assume xorriso or grub-mkrescue handles it or we need syslinux installed.
        # Fallback: simple grub-mkrescue if grub is available, else xorriso
        ("6. ISO", "BURN_ISO",
    ...
    for i, (cat, name, cmd) in enumerate(cards):
        uid = str(uuid.uuid4())
        # Tag payload with name for clarity in logs
