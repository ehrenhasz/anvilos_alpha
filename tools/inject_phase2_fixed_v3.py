...
CARDS = [
    ...
    # --- 3. FETCH & BUILD LIBTIRPC (Static) ---
    {
        "op": "SYS_CMD",
        "pld": {
            "cmd": "mkdir -p oss_sovereignty && cd oss_sovereignty && wget -O libtirpc-1.3.4.tar.bz2 https://downloads.sourceforge.net/project/libtirpc/libtirpc/1.3.4/libtirpc-1.3.4.tar.bz2 && tar -xf libtirpc-1.3.4.tar.bz2 && cd libtirpc-1.3.4 && ./configure --prefix=$(pwd)/../staging --enable-static --disable-shared --disable-gssapi && make -j$(nproc) && make install",
            "timeout": 300
        },
        "desc": "BUILD_LIBTIRPC"
    },

    # --- 4. BUILD LIBUUID & LIBBLKID (Static) ---
    {
        "op": "SYS_CMD",
        "pld": {
    ...
    # --- 5. BUILD ZFS USERLAND (With Explicit Flags) ---
    {
        "op": "SYS_CMD",
        "pld": {
...
print(f"[*] Injecting {len(CARDS)} Phase 2 (FIXED V3) Cards...")
