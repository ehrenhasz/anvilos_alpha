...
def inject():
    ...
    # Define Cards with Absolute Paths for Libtool Compatibility
    # REMOVED ./autogen.sh because it requires missing automake/autoconf
    # and the configure script is already present.
    cards = [
    ...
    for i, (name, cmd) in enumerate(cards):
        uid = str(uuid.uuid4())
        # Wrap cmd in JSON object for Processor Daemon
    ...
    conn.commit()
    conn.close()
    print(f"[*] Injecting {len(cards)} Phase 2 (FIXED V7) Cards...")
