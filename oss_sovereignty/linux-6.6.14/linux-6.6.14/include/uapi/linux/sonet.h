...
#define __SONET_ITEMS \
    __HANDLE_ITEM(section_bip); 	  \
    __HANDLE_ITEM(line_bip);		  \
    __HANDLE_ITEM(path_bip);		  \
    __HANDLE_ITEM(line_febe);		  \
    __HANDLE_ITEM(path_febe);		  \
    __HANDLE_ITEM(corr_hcs);		  \
    __HANDLE_ITEM(uncorr_hcs);		  \
...
#define SONET_GETFRSENSE _IOR('a',ATMIOC_PHYTYP+7, \
  unsigned char[SONET_FRSENSE_SIZE])	 
