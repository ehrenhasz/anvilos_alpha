...
static void michael_mic_hdr(struct sk_buff *skb, u8 *hdr)
{
	...
	case RTLLIB_FCTL_TODS:
		ether_addr_copy(hdr, hdr11->addr3);  
		ether_addr_copy(hdr + ETH_ALEN, hdr11->addr2);  
		break;
	case RTLLIB_FCTL_FROMDS:
		ether_addr_copy(hdr, hdr11->addr1);  
		ether_addr_copy(hdr + ETH_ALEN, hdr11->addr3);  
		break;
	case RTLLIB_FCTL_FROMDS | RTLLIB_FCTL_TODS:
		ether_addr_copy(hdr, hdr11->addr3);  
		ether_addr_copy(hdr + ETH_ALEN, hdr11->addr4);  
		break;
	case 0:
		ether_addr_copy(hdr, hdr11->addr1);  
		ether_addr_copy(hdr + ETH_ALEN, hdr11->addr2);  
